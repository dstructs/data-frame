...
*
* @param {Array} arr - rows to add
* @param {Object} [options] - options
* @param {Array} [options.names] - row names
* @param {Number} [options.idx] - row index at which to insert the rows
* @returns {DataFrame} DataFrame instance
*/
DataFrame.prototype.addRows = function( arr, options ) {
	...
	tmp = new Array( len );
	for ( i = 0; i < len; i++ ) {
		if ( arr[ i ].length > this._nCols ) {
			throw new Error( 'addRows()::invalid input argument. Row length does not equal the number of columns. Row index: ' + i + '.' );
		}
		tmp[ i ] = arr[ i ].slice();
	}
	// Add the rows to the data array...
	for ( i = 0; i < len; i++ ) {
		this._data.push( tmp[ i ] );
		this._nRows += 1;
